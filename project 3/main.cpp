...
int main() {
    ...
    if (!readConfigFile("files/config.cfg", columns, rows, numMines)) {
        return 1;
    }
    ...
    Digit digit("files/images/digits.png");

    std::vector<int> times;
    std::vector<std::string> names;
    int changed_pos = -1;
    readLeaderboardFile("files/leaderboard.txt", times, names);
                ...
                if (event.mouseButton.button == sf::Mouse::Left) {
                    if (happyface.handleClick(mouseX, mouseY)) {
                        ...
                        if(board.leftClick(mouseX/32, mouseY/32)){
                            ...
                            writeLeaderboardFile("files/leaderboard.txt", times, names);
                        }
                    }
